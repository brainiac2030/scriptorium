...
import { Clock3, NotebookPen, PencilLine, Trash2 } from 'lucide-react';
import BookCover from './BookCover';

const statusLabels = { to_read: 'Want to read', reading: 'Reading now', finished: 'Finished' };

function LibraryBookRow({ book, onStatusChange, onLogSession, onUpdateProgress, onOpenJournal, onRemove }) {
  ...
  return (
    <article className="group grid grid-cols-[64px_1fr] gap-4 border-t border-burgundy-900/15 py-5 sm:grid-cols-[72px_1fr_auto] sm:items-center sm:gap-5">
      ...
      <div className="col-span-2 flex flex-wrap items-center gap-2 pl-20 sm:col-span-1 sm:pl-0">
        <select value={book.status} onChange={(e) => onStatusChange(book.id, e.target.value)} className="h-10 rounded-lg border border-burgundy-900/20 bg-transparent px-3 text-xs font-semibold text-burgundy-900 outline-none focus:border-burgundy-700" aria-label={`Reading status for ${book.title}`}><option value="to_read">{statusLabels.to_read}</option><option value="reading">{statusLabels.reading}</option><option value="finished">{statusLabels.finished}</option></select>
        {book.status === 'reading' && <><button onClick={() => onUpdateProgress(book)} className="grid h-10 w-10 place-items-center rounded-lg border border-burgundy-900/20 text-burgundy-800 hover:bg-cream-200" title="Update page"><PencilLine className="h-4 w-4" /></button><button onClick={() => onLogSession(book)} className="grid h-10 w-10 place-items-center rounded-lg border border-burgundy-900/20 text-burgundy-800 hover:bg-cream-200" title="Log reading session"><Clock3 className="h-4 w-4" /></button></>}
        <button onClick={() => onOpenJournal(book)} className="grid h-10 w-10 place-items-center rounded-lg border border-burgundy-900/20 text-burgundy-800 hover:bg-cream-200" title="Open reading journal"><NotebookPen className="h-4 w-4" /></button>
        {onRemove && <button onClick={() => onRemove(book)} className="grid h-10 w-10 place-items-center rounded-lg border border-transparent text-gray-400 hover:border-red-200 hover:text-red-700" title="Remove book"><Trash2 className="h-4 w-4" /></button>}
      </div>
    </article>
  );
}
