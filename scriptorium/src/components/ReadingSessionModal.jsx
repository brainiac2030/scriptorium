...
function ReadingSessionModal({ isOpen, onClose, book, onSessionLogged }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      success('Reading session logged successfully');
      onSessionLogged();
      setFormData({ duration_minutes: '', pages_read: '', notes: '' });
      onClose();
    } catch (err) {
      const message =
        err.response?.data?.error || 'Failed to log reading session';
      setError(message);
      showError(message);
    } finally {
      setLoading(false);
      ...
      <div className="bg-white rounded-2xl shadow-lift max-w-md w-full p-6 md:p-8 relative animate-scaleIn">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            <div className="bg-burgundy-100 p-2.5 rounded-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, pages_read: e.target.value })
              }
              className="w-full px-4 py-3 border border-burgundy-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-burgundy-500"
              placeholder="e.g. 15"
            />
